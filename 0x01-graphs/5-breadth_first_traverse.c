...
size_t breadth_first_traverse(GRAPH, ACTION)
{
	...
	for (i = 0; vertices[i]; i++)
	{
		...
		for (p = vertices[i]->edges; p; p = p->next)
		{
			...
			{
				vertices[j++] = p->dest, seen[p->dest->index] = 1;
				next_round_size += 1;
			}
		}
		if (--round_size == 0)
		{
			depth++, round_size = next_round_size;
			next_round_size = 0;
		}
	}
	free(seen), free(vertices);
	return (depth - 1);
}
